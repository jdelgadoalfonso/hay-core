...
export function useMessageSync(options: SyncOptions) {
  ...
  const sync = async () => {
    ...
    try {
      ...
      console.log('[MessageSync] Starting sync for conversation:', conversationId);
      ...
      if (missingMessages.length > 0) {
        console.log(
          `[MessageSync] Found ${missingMessages.length} missing messages, syncing...`,
        );
      ...
      if (extraMessages.length > 0) {
        console.warn(
          `[MessageSync] Found ${extraMessages.length} extra local messages (may be pending):`,
  ...
  const startSync = () => {
    ...
    console.log(`[MessageSync] Starting periodic sync (interval: ${syncInterval}ms)`);
  ...
  const stopSync = () => {
    if (syncTimer) {
      ...
      syncTimer = null;
      console.log('[MessageSync] Periodic sync stopped');
    }
  };
