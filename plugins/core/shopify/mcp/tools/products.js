...
function registerProductTools(server) {
  server.tool(
    ...
    "Get a single product by id (GID or numeric) including variants.",
    {
      ...
      variantsFirst: z
        ...
        .int()
        .optional()
        .describe("Number of variants to fetch (default 50)"),
    },
    async (args) => {
      try {
        const variables = {
  ...
  server.tool(
    ...
    async (args) => {
      try {
        // Verified 2026-04: Shopify search supports only trailing prefix wildcards
        // (`title:shirt*`), not surrounding/leading wildcards (`*shirt*`).
