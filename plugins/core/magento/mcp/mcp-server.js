  ...
  async ({ sku }) => {
    try {
      ...
      const categoryPromises = categoryIds.map((id) =>
        callMagentoApi(`/categories/${id}`).catch((err) => ({ id, error: err.message })),
      );
...
server.tool(
  ...
  "Search for products with advanced filtering options",
  {
    ...
    condition_type: z
      .string()
      .optional()
      .describe("Condition type (eq, like, gt, lt, etc.). Default: eq"),
    page_size: z.number().optional().describe("Number of results per page (default: 10)"),
    current_page: z.number().optional().describe("Page number (default: 1)"),
  ...
  async ({
    ...
    try {
      ...
      const searchCriteria =
        `searchCriteria[filter_groups][0][filters][0][field]=${encodeURIComponent(field)}&` +
...
server.tool(
  ...
  "Update a specific attribute of a product by SKU",
  {
    ...
    attribute_code: z
      .string()
      .describe(
        "The code of the attribute to update (e.g., name, price, description, status, etc.)",
      ),
...
server.tool(
  ...
  "Get the total revenue for a given date range",
  {
    date_range: z
      .string()
      .describe(
        "Date range expression (e.g., 'today', 'yesterday', 'last week', 'this month', 'YTD', or a specific date range like '2023-01-01 to 2023-01-31')",
      ),
    status: z
      .string()
      .optional()
    ...
    include_tax: z
      .boolean()
      .optional()
      .describe("Whether to include tax in the revenue calculation (default: true)"),
  },
  async ({ date_range, status, include_tax = true }) => {
    try {
      ...
      let searchCriteria = buildDateRangeFilter(
        "created_at",
        dateRange.startDate,
        dateRange.endDate,
      );
      ...
      const result = {
        ...
        result: {
          ...
          average_order_value:
            orderCount > 0 ? parseFloat((finalRevenue / orderCount).toFixed(2)) : 0,
...
server.tool(
  ...
  "Get statistics about the quantity of products sold in a given date range",
  {
    ...
    country: z
      ...
      .optional()
      .describe(
        "Filter by country code (e.g., 'US', 'NL', 'GB') or country name (e.g., 'United States', 'The Netherlands', 'United Kingdom')",
      ),
  },
    ...
    try {
      ...
      if (country) {
        ...
        filteredOrders = filteredOrders.filter((order) => {
          ...
          return (
            normalizedCountry.includes(billingCountry) ||
            normalizedCountry.includes(shippingCountry)
          );
        });
      }
      ...
      const result = {
        ...
        result: {
          ...
          average_products_per_order:
            totalOrders > 0 ? parseFloat((totalProductQuantity / totalOrders).toFixed(2)) : 0,
          total_revenue: parseFloat(totalRevenue.toFixed(2)),
          average_revenue_per_product:
            totalProductQuantity > 0
              ? parseFloat((totalRevenue / totalProductQuantity).toFixed(2))
              : 0,
...
server.tool(
  ...
  "Get revenue filtered by country for a given date range",
  {
    ...
    country: z
      .string()
      .describe(
        "Country code (e.g., 'US', 'NL', 'GB') or country name (e.g., 'United States', 'The Netherlands', 'United Kingdom')",
      ),
    status: z
      .string()
      .optional()
